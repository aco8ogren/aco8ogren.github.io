...
(() => {
  ...
  const elClear = document.getElementById('pub-clear');
  ...
  const getDefaultOrder = (p) => p.defaultOrder || 0;
  function toArray(v) {
  ...
  const sorters = {
    'default': (a, b) => getDefaultOrder(b) - getDefaultOrder(a),
  ...
  function updateFilterSummary() {
    ...
    if (activeLabels.length || qRaw) {
      frag.append('Showing publications that ')
    } else {
      frag.append('Showing all publications')
    };
  ...
  function clearFilters() {
    ...
    apply();                 // re-render list, kw bar, summary, etc.
  }

  function syncClearState() {
    if (!elClear) return;
    const hasSearch = !!(elSearch && elSearch.value.trim());
    const hasKws = activeKws.size > 0;
    elClear.disabled = !(hasSearch || hasKws);
  }
  ...
  function apply() {
    ...
    const sorter = sorters[elSort.value] || sorters['default'];
  ...
  (async () => {
    ...
    elSearch.addEventListener('input', syncClearState);
